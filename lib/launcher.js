...
exports.launchLiteServ = function(opts) {
  ...
  if (process.env.REVS_LIMIT && process.env.LITESERV_MACOSX_VERSION &&
      process.env.LITESERV_MACOSX_VERSION.substring(0, 3) >"1.2") {
    console.log("USING CUSTOM REVS_LIMIT!! " + process.env.REVS_LIMIT)
    argv.push("--revs_limit");
    argv.push(process.env.REVS_LIMIT);
  }
  ...
  if (config.storageEngine) {
	...
	if (config.provides == "android") {
        console.info("Launching ", config.LiteServPath)
