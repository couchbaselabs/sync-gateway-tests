...
exports.launchSyncGatewayCommon = function(path, config, params){
    ...
    sg.stderr.on("data",function(data) {
        if ((data.toString().indexOf("Starting server on :" + port) !== -1) ||
            (data.toString().indexOf("Starting server on localhost:" + port) !== -1)) {
            setTimeout(function () {
                sg.emit("ready")
            }, (function () {
                return 1000;
...
//		var liteserv = spawn(opts.path + '/run_android_liteserv.sh', [ opts.port ]);
		var liteserv =spawn('adb', [ "shell", 'am start -a android.intent.action.MAIN -n com.couchbase.liteservandroid/com.couchbase.liteservandroid.MainActivity --es username "" --es password "" --ei listen_port', 
		                             opts.port]);
